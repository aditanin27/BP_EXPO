...
const FormAnakScreen = ({ route, navigation }) => {
  const { idAnak } = route.params || {};
  const isEditing = !!idAnak;
  
  const dispatch = useAppDispatch();
  const { selectedAnak, isLoading, error, createSuccess, updateSuccess } = useAppSelector((state) => state.anak);
  ...
  // Load anak data if editing
  useEffect(() => {
    if (isEditing) {
      dispatch(fetchAnakById(idAnak));
    }
  }, [dispatch, isEditing, idAnak]);

  // Populate form when anak data is available
  useEffect(() => {
    if (isEditing && selectedAnak) {
      setFormData({
        full_name: selectedAnak.full_name || '',
        nick_name: selectedAnak.nick_name || '',
        nik_anak: selectedAnak.nik_anak || '',
        tempat_lahir: selectedAnak.tempat_lahir || '',
        tanggal_lahir: formatDateForInput(selectedAnak.tanggal_lahir) || '',
        jenis_kelamin: selectedAnak.jenis_kelamin || 'L',
        agama: selectedAnak.agama || '',
        anak_ke: selectedAnak.anak_ke ? selectedAnak.anak_ke.toString() : '',
        dari_bersaudara: selectedAnak.dari_bersaudara ? selectedAnak.dari_bersaudara.toString() : '',
        tinggal_bersama: selectedAnak.tinggal_bersama || '',
        hobi: selectedAnak.hobi || '',
        pelajaran_favorit: selectedAnak.pelajaran_favorit || '',
        prestasi: selectedAnak.prestasi || '',
        hafalan: selectedAnak.hafalan || '',
  ...
  const handleSubmit = () => {
    if (validateForm()) {
      ...
      if (isEditing) {
        dispatch(updateAnak({ id: idAnak, dataAnak: processedData }));
      } else {
        dispatch(createAnak(processedData));
      }
    }
  };
              ...
              <Image
                source={{ 
                  uri: formData.foto 
                    ? formData.foto.uri 
                      ? formData.foto.uri 
                      : `${IMAGE_BASE_URL}Anak/${idAnak}/${formData.foto}`
